...
export default {
  ...
  methods: {
    load() {
      this.axios.get('/api/irregular-cost').then((response) => {
    ...
    save() {
      if (this.form.id >= 0) {
        this.axios.put('/api/irregular-cost', this.form).then((response) => {
          this.dialogFormVisible = false;
          this.load();
        })
      } else {
        this.axios.post('/api/irregular-cost', this.form).then((response) => {
          this.dialogFormVisible = false;
          this.load();
        })
      }
    ...
    handleDelete($index, row) {
      this.form = JSON.parse(JSON.stringify(row));
      this.axios.delete('/api/irregular-cost/' + this.form.id).then((response) => {
        this.dialogFormVisible = false;
        this.load();
      })
    },
