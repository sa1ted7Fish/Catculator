...
export default {
  ...
  methods: {
    load() {
      this.axios.get('/api/initial-cost').then((response) => {
    ...
    save() {
      if (this.form.id >= 0) {
        this.axios.put('/api/initial-cost', this.form).then((response) => {
          this.dialogFormVisible = false;
          this.load();
        })
      } else {
        this.axios.post('/api/initial-cost', this.form).then((response) => {
          this.dialogFormVisible = false;
          this.load();
        })
      }
    ...
    handleDelete($index, row) {
      this.form = JSON.parse(JSON.stringify(row));
      this.axios.delete('/api/initial-cost/' + this.form.id).then((response) => {
        this.dialogFormVisible = false;
        this.load();
      })
    },
